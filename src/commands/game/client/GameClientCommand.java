 ...
 * post a response (player action, e.g. use Dodge). If a player is allowed to post a response, 
 * then they will receive a UUID generated on the server side which must be sent back
 * along with the response command so the server may accept the response.
 * 
