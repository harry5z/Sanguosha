...
public class Damage implements Command
{
	 ...
	 */
	public Damage(Card cardUsed, PlayerInfo source, PlayerInfo target, Command next)
	{
		super(next);
	 ...
	 * @param target : target of damage
	 */
	public Damage(int amount, Element element, PlayerInfo source, PlayerInfo target, Command next)
	{
		super(next);
		this.amount = amount;
	...
	private void next(PlayerComplete player)
	{
		stage++;
	...
	public void ClientOperation(PlayerComplete player)
	{
		...
		if(player.equals(target))
		{
			...
			{	
				...
				{
					stage++;
					player.sendToServer(new NearDeathOperation(player.getCurrentStage().getSource(),source,target,this));
				}
				else
					next(player);
				return;
